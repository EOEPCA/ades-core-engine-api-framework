...
#define T2CWL_IMOD_HPP
    ...
    public:
        ...
        explicit ArgoInterface(const std::string &path) : mods::IModInterface(path), start(nullptr), getStatus(nullptr), getResults(nullptr) {

            setValid(true);

            start = (int (*)(ArgoWorkflowConfig &awConfig, const std::string &cwlContent, std::list<std::pair<std::string, std::string>> &inputList, const std::string &uuidBaseID, const std::string &runId, std::string &id)) dlsym(handle, "start");
            if (!start) {
                std::cerr << "start\n";
                setValid(false);
                return;
            ...
            getStatus = (int (*)(ArgoWorkflowConfig &awConfig, const std::string &argoWorkfloId, int &percent, std::string &message)) dlsym(handle, "getStatus");
            if (!getStatus) {
                setValid(false);
                std::cerr << "getStatus\n";
                return;
            }
            getResults = (int (*)(ArgoWorkflowConfig &awConfig, const std::string &argoWorkfloId, std::list<std::pair<std::string, std::string>> &outPutList)) dlsym(handle, "getResults");
            if (!getResults) {
                std::cerr << "getResults\n";
                setValid(false);
                return;
    ...
    public:
         ...
         * @return 0 ok  >0 notOk
         */
        int (*start)(ArgoWorkflowConfig &awConfig, const std::string &cwlContent, std::list<std::pair<std::string, std::string>> &inputList, const std::string &uuidBaseID, const std::string &runId, std::string &id);
         ...
         */
        int (*getStatus)(ArgoWorkflowConfig &awConfig, const std::string &argoWorkfloId, int &percent, std::string &message);
        int (*getResults)(ArgoWorkflowConfig &awConfig, const std::string &argoWorkfloId, std::list<std::pair<std::string, std::string>> &outPutList);
    };
